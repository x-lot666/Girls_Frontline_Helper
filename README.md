...
- **2025.12.14**: v1.1.0.3(测试版)
    - 修复:任务序列时各种卡住的问题

- **2025.12.04**: v1.1.0.2(测试版)
    - 修复:
      - 总控执行一个任务后退出进程
      - 打捞出新人形卡住
      - 主界面回收人形后,错误点击钻石充值界面

- **2025.12.04**: v1.1.0.1(测试版)
    - 新增功能:
        - 添加总控，用于统一管理与调度所有关卡脚本，原本每个脚本都需要独立执行，现在可以通过一个总控入口统一管理，
          通过配置`mission_plan.yaml`文件，然后运行`launcher.py`
        - 日志系统重构，日志统一保存在项目根目录下的logs文件夹内，并自动清理旧日志（仅保留最新 10 条）
        - 打捞前会先进行一遍人形回收(仅二星)
    - 修复:上个版本打捞出现的各种问题:清理人形后卡在主界面等
...
总控脚本用于统一调度所有关卡脚本，让整个项目不再需要“逐个进入脚本文件、修改参数、手动运行”。现在只需要编辑一个配置文件就能完成复杂的自动化任务流程。

1. 执行入口只有一个，`game_scripts/launcher.py`，运行之后程序会自动加载 `game_scripts/mission_plan.yaml`
2. `mission_plan.yaml` 配置说明:一个任务包含三个字段`type`、`level`、`args`,可以配置多个任务,启动后从上到下依次执行

| 字段      | 类型     | 说明                                                    |
|---------|--------|-------------------------------------------------------|
| `type`  | string | 脚本文件夹的名称，(日常战役对应`main_mission`/`活动战役对应event_mission`) |
| `level` | string | 脚本文件名：如`a_normal`（不含 .py）                             |
| `args`  | dict   | 对应脚本的参数，具体填什么需要看具体脚本的主函数 `main(**params)`             |
...
| 总控调度模块    | `launcher.py` 统一管理与调度所有脚本，基于 `mission_plan.yaml` 自动加载并执行任务流程，实现“一键执行所有关卡脚本”的自动化流程 |
